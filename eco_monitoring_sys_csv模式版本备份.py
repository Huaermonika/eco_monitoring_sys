...
class CSVManager(object): # CSV 文件管理
    ...
    def save_record_csv(self,record):
        """
        保存一条记录到 CSV 文件中，并自动分配唯一的 record_id
        
        Args:
            record (MonitorRecord): 待保存的记录对象
        """
        max_id = 0 # 记录最大 ID
        ...
        if os.path.exists(self.filepath):
            with open(self.filepath, mode='r', encoding='utf-8') as file:
                ...
                header = next(reader, None) #指针操作,安全跳过标题行
                for row in reader:
                    if row:
